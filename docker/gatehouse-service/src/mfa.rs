...
mod tests {
    ...
    fn with_key<T>(run: impl FnOnce() -> T) -> T {
        envmnt::set("GATEHOUSE_KEY_ENCRYPTION_KEY", "test-key-material");
        run()
    }
    ...
    fn a_generated_secret_verifies_its_own_current_code() {
        ...
        let code = totp(parsed, "someone").unwrap().generate_current();
    ...
    fn a_code_for_a_different_secret_does_not_verify() {
        ...
        let code = totp(other_parsed, "someone").unwrap().generate_current();

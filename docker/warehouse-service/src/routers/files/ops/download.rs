...
//! large artifact does not cost its size in memory - and several concurrent
//! downloads do not cost their combined size.

use super::{error, not_found, target_or_error};
...
#[head("/{storage}/file")]
#[tracing::instrument]
pub async fn head(storage: web::Path<String>, query: web::Query<FileQuery>) -> impl Responder {
    let storage_name = storage.into_inner();
    let (_, target) = match target_or_error(&storage_name, &query.path).await {
        Ok(resolved) => resolved,
        Err(response) => return response,

...
/// Guards `CRATES_STORAGE_PATH`/`STORAGE_PATH`, read fresh on every call by
/// `warehouse_service::routers::crates_storage_root`/`docker_storage_root` -
/// two tests setting different values concurrently would otherwise race each
/// other's storage roots out from under them.

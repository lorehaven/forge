use crate::support;

use http::{Method, StatusCode};

const DIGEST: &str = "sha256:e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855";

async fn app() -> (
    std::sync::Arc<dyn quench_http::endpoint::Endpoint>,
    std::sync::Arc<quench_http::di::Container>,
) {
    warehouse_service::routers::docker::blob::start_upload::register_routes();
    let container = support::container_builder().build().await.unwrap();
    support::app(container).await
}

#[tokio::test]
async fn handle_rejects_an_invalid_repository_name() {
    let _storage = support::WithDockerStorageRoot::new();
    let (app, container) = app().await;
    let resp = app
        .call(support::req(
            Method::POST,
            "/v2/..%2fetc/blobs/uploads/",
            &container,
        ))
        .await;
    assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
}

#[tokio::test]
async fn handle_starts_a_regular_upload_and_creates_the_staging_file() {
    let storage = support::WithDockerStorageRoot::new();
    let (app, container) = app().await;
    let resp = app
        .call(support::req(
            Method::POST,
            "/v2/my-repo/blobs/uploads/",
            &container,
        ))
        .await;
    assert_eq!(resp.status(), StatusCode::ACCEPTED);
    let (headers, _) = support::parts(resp).await;
    assert_eq!(headers.get("range").unwrap(), "0-0");
    assert!(headers.contains_key("docker-upload-uuid"));

    let uploads_dir = storage.dir.path().join("my-repo").join("_uploads");
    assert!(uploads_dir.exists());
    assert_eq!(std::fs::read_dir(&uploads_dir).unwrap().count(), 1);
}

#[tokio::test]
async fn handle_rejects_a_cross_repo_mount_with_an_invalid_digest() {
    let _storage = support::WithDockerStorageRoot::new();
    let (app, container) = app().await;
    let resp = app
        .call(support::req(
            Method::POST,
            "/v2/my-repo/blobs/uploads/?mount=not-a-digest&from=other-repo",
            &container,
        ))
        .await;
    assert_eq!(resp.status(), StatusCode::BAD_REQUEST);
}

#[tokio::test]
async fn handle_mounts_an_existing_blob_from_another_repository() {
    let storage = support::WithDockerStorageRoot::new();
    let hex = DIGEST.strip_prefix("sha256:").unwrap();
    let blob_dir = storage.dir.path().join("blobs").join("sha256");
    std::fs::create_dir_all(&blob_dir).unwrap();
    std::fs::write(blob_dir.join(hex), b"content").unwrap();

    let (app, container) = app().await;
    let resp = app
        .call(support::req(
            Method::POST,
            &format!("/v2/my-repo/blobs/uploads/?mount={DIGEST}&from=other-repo"),
            &container,
        ))
        .await;
    assert_eq!(resp.status(), StatusCode::CREATED);
    let (headers, _) = support::parts(resp).await;
    assert_eq!(headers.get("docker-content-digest").unwrap(), DIGEST);
}

#[tokio::test]
async fn handle_falls_back_to_a_regular_upload_when_the_mount_target_does_not_exist() {
    let _storage = support::WithDockerStorageRoot::new();
    let (app, container) = app().await;
    let resp = app
        .call(support::req(
            Method::POST,
            &format!("/v2/my-repo/blobs/uploads/?mount={DIGEST}&from=other-repo"),
            &container,
        ))
        .await;
    assert_eq!(resp.status(), StatusCode::ACCEPTED);
}

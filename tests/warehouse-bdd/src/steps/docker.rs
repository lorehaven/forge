...
async fn send_get_request_with_token(world: &mut WarehouseWorld, path: String) {
...
async fn send_put_request_with_token_valid_manifest(world: &mut WarehouseWorld, path: String) {
...
async fn check_tag_in_list(world: &mut WarehouseWorld, tag: String) {
    let json = world
        .last_json
        .as_ref()
...
async fn check_header(world: &mut WarehouseWorld, header: String) {
...
async fn delete_by_header_digest(world: &mut WarehouseWorld, repo: String, _header: String) {
    ...
    let url = format!("{}/v2/{}/manifests/{}", world.base_url, repo, digest);
    let mut rb = world.client.delete(&url);
    if let Some(token) = &world.token {
        rb = rb.bearer_auth(token);
    }
...
async fn get_token_step(world: &mut WarehouseWorld, service: String, scope: String) {
    let url = format!(
        "{}/token?service={}&scope={}",
        world.base_url, service, scope
    );
...
async fn check_token(world: &mut WarehouseWorld) {
    assert!(world.token.is_some(), "Token not found in response");
}

#[cucumber::given(expr = "valid token for scope {string} is obtained")]
async fn have_token(world: &mut WarehouseWorld, scope: String) {
    let url = format!("{}/token?service=warehouse&scope={}", world.base_url, scope);
    let auth = STANDARD.encode(format!("{}:{}", world.username, world.password));
    let res = world
        .client
        .get(&url)
...
async fn check_tags(world: &mut WarehouseWorld) {
    let json = world
        .last_json
        .as_ref()
        .expect("No JSON response available");

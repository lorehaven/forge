...
async fn get_without_token(world: &mut WarehouseWorld, page: String) {
...
async fn get_with_malformed_token(world: &mut WarehouseWorld, page: String) {
    let url = format!("{}{}", world.api_url, page);
    let client = reqwest::Client::builder()
        .danger_accept_invalid_certs(true)
        .redirect(reqwest::redirect::Policy::none())
    ...
    let res = client
        .get(&url)
        .header(COOKIE, "warehouse_ui_session=not-a-valid-token".to_string())
        .send()
        .await
        .expect("Failed to send GET request");
...
async fn get_with_wrong_secret(world: &mut WarehouseWorld, page: String) {
    ...
    let res = client
        .get(&url)
        .header(COOKIE, format!("warehouse_ui_session={}", token))
        .send()
        .await
        .expect("Failed to send GET request");
...
async fn get_with_expired_token(world: &mut WarehouseWorld, page: String) {
...
async fn get_with_wrong_service(world: &mut WarehouseWorld, page: String, service: String) {
    let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| "secret".to_string());

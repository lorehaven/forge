...
async fn obtain_token(world: &mut ForgeWorld) {
...
async fn publish_crate(world: &mut ForgeWorld, name: String, version: String) {
    ...
    let url = format!("{}/api/v1/crates/new", world.warehouse_url);
...
fn resolve_crate_path(world: &ForgeWorld, path: String) -> String {
    ...
    if resolved_path.starts_with("/api/v1/crates") || resolved_path.starts_with("/index") {
        format!("{}{}", world.warehouse_url, resolved_path)
    } else {
        format!("{}{}", world.warehouse_base_url, resolved_path)
    }
}
...
async fn send_put_request_with_token(world: &mut ForgeWorld, path: String) {
    let url = resolve_crate_path(world, path);
...
async fn publish_crate_no_token(world: &mut ForgeWorld, name: String, version: String) {
    let url = format!("{}/api/v1/crates/new", world.warehouse_url);
...
async fn publish_crate_invalid_metadata(world: &mut ForgeWorld, step: &cucumber::gherkin::Step) {

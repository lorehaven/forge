...
async fn request_available_models(world: &mut SageWorld) -> Result<(), String> {
    let url = format!("{}/sage/api/v1/models", world.api_base_url);
...
async fn default_model_not_running(_world: &mut SageWorld) -> Result<(), String> {
    // In mock setup, models start not running
    Ok(())
}

#[cucumber::then("sage should request switchboard to launch the model")]
async fn sage_should_launch_model(_world: &mut SageWorld) -> Result<(), String> {
    Ok(())
}

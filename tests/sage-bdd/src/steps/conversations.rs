...
async fn create_conversation(world: &mut SageWorld) -> Result<(), String> {
...
async fn have_conversation(world: &mut SageWorld) -> Result<(), String> {
    create_conversation(world).await
}

#[cucumber::given("I have a conversation with messages")]
async fn have_conversation_with_messages(world: &mut SageWorld) -> Result<(), String> {
    create_conversation(world).await?;
...
async fn send_to_conversation(world: &mut SageWorld, message: String) -> Result<(), String> {
    let url = format!("{}/sage/api/v1/chat", world.api_base_url);
    let conv_id = world
        .current_conversation_id
        .clone()
...
async fn response_contains_conversation_id(world: &mut SageWorld) -> Result<(), String> {
...
async fn response_contains_message(_world: &mut SageWorld) -> Result<(), String> {
    // Placeholder - in real scenario would parse response
    Ok(())
}

#[cucumber::when("I retrieve the conversation")]
async fn retrieve_conversation(world: &mut SageWorld) -> Result<(), String> {
    let conv_id = world
        .current_conversation_id
        .clone()
        .ok_or("No conversation created")?;
...
async fn response_contains_all_messages(_world: &mut SageWorld) -> Result<(), String> {
    // Placeholder - in real scenario would validate messages in response
    Ok(())
}

#[cucumber::given("I have multiple conversations")]
async fn have_multiple_conversations(world: &mut SageWorld) -> Result<(), String> {
    for _ in 0..3 {
        create_conversation(world).await?;
    }
    Ok(())
...
async fn list_conversations(world: &mut SageWorld) -> Result<(), String> {
    let url = format!("{}/sage/api/v1/conversations", world.api_base_url);
...
async fn response_contains_conversations(_world: &mut SageWorld) -> Result<(), String> {
...
async fn each_message_has_response(_world: &mut SageWorld) -> Result<(), String> {
    Ok(())
}

...
/// prompt history.
///
/// `read_file`/`search` can return output far larger than there's room for
/// once it's resent on every subsequent step.
pub const MAX_TOOL_OUTPUT_CHARS: usize = 4_000;

/// Total history size cap.
///
/// Every step's history is resent as part of the prompt on every following
/// step, so left unbounded it grows the prompt by O(steps²) in the worst
/// case. Once this is exceeded, the oldest entries are dropped from the
/// front, keeping the most recent (and most relevant) context.
pub const MAX_HISTORY_CHARS: usize = 12_000;

#[must_use]
pub fn truncate_for_history(text: &str) -> std::borrow::Cow<'_, str> {
...
pub fn cap_history(history: &mut String) {
...
#[must_use]
pub fn resolve_run_cmd_allowlist(
    node: &AgentNode,
    indexed_files: Option<&[String]>,
) -> Vec<String> {
...
#[must_use]
pub fn detect_tech_stack(files: &[String]) -> Vec<String> {
...
#[must_use]
pub fn default_allowlist_for_stack(detected: &[String]) -> Vec<String> {

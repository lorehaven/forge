...
- **CPU launches** — a launch request's optional `device` field selects the execution device. Omitted / `"gpu"` / `"auto"` keeps the historical behaviour (vLLM auto-selects the accelerator). `"cpu"` drops `--gpu-memory-utilization` (meaningless on CPU) and sets `VLLM_CPU_KVCACHE_SPACE` (GiB, from the env var or a default of 4); CPU execution itself comes from the *runtime*, not a flag — a CPU-only vLLM build (native mode) or the `vllm-openai-cpu` image (Kubernetes mode). No `--device cpu` is emitted: current vLLM repurposed `--device` for GPU device *ids* and rejects `--device cpu`. An explicit non-CPU accelerator name (`"cuda"`, `"neuron"`, …) is still passed verbatim as `--device <value>`. CPU inference cannot load FP8 checkpoints. GPU launches are entirely unaffected — the two run side by side.
  ...
  - The launch modal exposes this as a GPU/CPU selector and, for CPU, disables the GPU-utilization field and skips the VRAM fit note. The running-instances grid tags each card with a `GPU`/`CPU` badge and, for a CPU instance, shows `Device: CPU` in place of the (meaningless) GPU-utilization figure.
